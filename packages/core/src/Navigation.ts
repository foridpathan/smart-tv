...
class NavigationService {
  ...
  smartNavigate(
    direction: string,
    fromParentFocusKey: string | null | undefined,
    ...
    if (currentComponent) {
      ...
      if (!layout) {
        // If layout is undefined, skip navigation logic
        return;
      }
  ...
  getNextFocusKey(targetFocusKey: string): string {
    ...
    if (children.length > 0) {
      ...
      if (childClosest) {
        const { focusKey: childKey } = childClosest;
        this.log("getNextFocusKey", "childKey will be focused", childKey);
        return this.getNextFocusKey(childKey);
      }

      /**
       * If no child is found, just return targetFocusKey back
       */
      this.log("getNextFocusKey", "No child found, returning targetFocusKey", targetFocusKey);
      return targetFocusKey;
    }
  ...
  getNodeLayoutByFocusKey(focusKey: string): FocusableComponentLayout {
    ...
    const defaultLayout: FocusableComponentLayout = {
      x: 0,
      y: 0,
      width: 0,
      height: 0,
